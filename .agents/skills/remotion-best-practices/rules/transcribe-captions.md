...
To transcribe audio to generate captions in Remotion, you can use the
[`transcribe()`](https://www.remotion.dev/docs/install-whisper-cpp/transcribe)
function from the
...
First, the @remotion/install-whisper-cpp package needs to be installed. If it is
not installed, use the following command:
...
Make a Node.js script to download Whisper.cpp and a model, and transcribe the
audio.
...
See [Displaying captions](display-captions.md) for how to display the captions
in Remotion.
