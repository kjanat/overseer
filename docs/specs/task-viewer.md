...
Agents using Overseer via MCP have no visual way to inspect the task store
state. Debugging task hierarchies, blocked dependencies, and context inheritance
requires raw CLI commands. A local webapp would provide:
...
**Architecture Decision:** Single-package Hono + Vite SPA (simplified from
original Astro spec). Astro's partial hydration benefits don't justify
complexity for a local-only dashboard with heavy React interactivity.
...
- `ui.rs:107`: Change detection from `"Local:"` to `"listening on"` or
  `"http://localhost"`
