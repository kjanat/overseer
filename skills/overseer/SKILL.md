...
Overseer tasks are **tickets** - structured artifacts with comprehensive
context:
...
Think: "Would someone understand the what, why, and how from this task alone AND
what success looks like?"

## Task IDs are Ephemeral

**Never reference task IDs in external artifacts** (commits, PRs, docs). Task
IDs like `task_01JQAZ...` become meaningless once tasks complete. Describe the
work itself, not the task that tracked it.
...
Use **Overseer** for persistent work. Use **TodoWrite** for ephemeral in-session
tracking only.
...
**Use `nextReady()`** when starting work - returns `TaskWithContext | null`
(deepest ready leaf with full context chain + inherited learnings). **Use
`list({ ready: true })`** for status/progress checks - returns `Task[]` without
context chain.
...
// 3. Start work (VCS required - jj: creates bookmark + checkout; git: records state only)
await tasks.start(task.id);

// 4. Implement...

// 5. Complete with learnings (VCS required - jj: commits + deletes bookmark; git: records SHA only)
...
Three levels: **Milestone** (depth 0) -> **Task** (depth 1) -> **Subtask**
(depth 2).
...
Your result must include explicit verification evidence. See @file
