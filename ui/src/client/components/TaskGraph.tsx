...
import { getStatusLabel, getStatusVariant } from '../lib/utils.js';
...
function getLayoutedElements(
	...
	const rootNode = milestones.length > 1 ? dagreGraph.node(VIRTUAL_ROOT_ID) : null;
...
const TaskNodeComponent = memo(function TaskNodeComponent({
	...
	const depthLabel = task.depth === 0 ? 'MILESTONE' : task.depth === 1 ? 'TASK' : 'SUBTASK';
	...
	return (
        ...
        `}
			...
			>
              ...
              `}
							style={{
								backgroundColor: statusVariant === 'archived'
					...
					<div className='flex items-center gap-1.5'>
						...
						{isNextUp && <Badge variant='nextUp'>Next Up</Badge>}
					</div>
				</div>
...
export function TaskGraph({
	...
	const [collapsedIds, setCollapsedIds] = useState<Set<TaskId>>(() => getDefaultCollapsedIds(tasks));
	...
	return (
		...
		>
			...
			<div
				...
				aria-hidden='true'
			>
				<span className='opacity-70'>h/l</span> collapse/expand <span className='opacity-70'>j/k</span> navigate{' '}
