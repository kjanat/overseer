...
import { computeExternalBlockerCounts, getDepthLabel, getStatusLabel, getStatusVariant } from '../lib/utils.js';
	...
	return (
		<div className='flex flex-col h-full' {...scopeProps}>
			...
			<div className='px-2 py-1 border-t border-border text-xs text-text-dim font-mono'>
				<span className='opacity-70'>j/k</span> navigate <span className='opacity-70'>h/l</span> collapse/expand{' '}
...
function TaskTreeNode({
	...
	const children = (tasksByParent.get(task.id) ?? []).filter((t) => visibleTaskIds.has(t.id));
	...
	return (
        ...
        ${
				isSelected
					...
					: isFocused
					? 'bg-surface-primary ring-1 ring-inset ring-border'
