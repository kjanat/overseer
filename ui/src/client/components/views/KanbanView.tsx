	...
	const moveLeft = useCallback(() => {
		...
		for (let i = focusedColumn - 1; i >= 0; i--) {
			...
			if (col && tasksByColumn[col].length > 0) {
				setFocusedColumn(i);
				setFocusedIndexInColumn((prevIdx) => Math.min(prevIdx, tasksByColumn[col].length - 1));
				return;
			}
		}
	...
	return (
		...
		>
			...
			<div className='px-4 py-2 border-t border-border text-xs text-text-dim font-mono flex-shrink-0'>
				<span className='opacity-70'>h/l</span> columns <span className='opacity-70'>j/k</span> navigate{' '}
